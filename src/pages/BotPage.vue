      ...
      <div class="h-full overflow-hidden max-md:h-auto">
        ...
        >
          ...
          >
            ...
            <!-- 업로드 완료 결과 -->
            <div
              v-if="batchId && uploadedItems.length > 0"
              class="mt-4 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl"
            >
              <div class="flex justify-between items-center mb-2">
                ...
                >
                <span
                  class="text-[10px] px-2 py-0.5 bg-emerald-500/20 text-emerald-300 rounded font-mono"
                  >{{ batchId }}</span
                >
              </div>
              <ul class="flex flex-col gap-1">
                ...
                >
                  <span class="text-slate-400">{{ item.original }}</span>
                  <span class="text-emerald-300 font-mono">{{ item.id }}</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <!-- Batch ID 입력 (로컬 원고 모드) -->
          <div v-if="!autoGenerateDraft" class="p-5 px-6">
            <div class="flex flex-col gap-1.5">
              <label
                class="text-[11px] font-medium text-slate-400 uppercase tracking-wide"
                >Batch ID (발행용)</label
              ...
              <div class="flex gap-2">
                <input
                  v-model="batchId"
                  type="text"
                  class="flex-1 h-10 px-3 bg-slate-900/60 border border-white/10 rounded-lg text-sm text-slate-200 font-mono placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all"
                  placeholder="업로드 후 자동 입력 또는 직접 입력"
                />
                <Button
                  v-if="batchId"
                  size="sm"
                  color="light"
                  class="px-3"
                  @click="batchId = null; uploadedItems = []"
                  title="초기화"
                  >×</Button
                >
              </div>
              <span
                ...
                >{{ uploadedItems.length }}개 원고 발행 대기</span
              >
            </div>
          </div>
